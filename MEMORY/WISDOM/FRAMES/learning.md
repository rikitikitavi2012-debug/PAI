# Frame: Learning Domain

## Meta
- **Domain:** learning
- **Confidence:** 50%
- **Observation Count:** 2
- **Last Crystallized:** 2026-03-01
- **Source:** Converted from learning.json

---

## Core Principles

*No crystallized principles yet. Observations accumulating.*

---

## Contextual Rules

- Ivan responds well to 10-topic structured onboarding with cheat sheet at the end — covers full PAI architecture in one session (learned 2026-02-28)

---

## Predictive Model

| Request Pattern | Predicted Want | Confidence |
|----------------|---------------|------------|

---

## Anti-Patterns (from observations)

### Pure theoretical explanations don't satisfy Ivan — he wants live examples and concrete demonstrations of how tools work, not abstract architecture
- **Severity:** Medium
- **Frequency:** Confirmed 1 times
- **Since:** 2026-02-26


---

## Cross-Frame Connections

*To be discovered through cross-frame synthesis.*

---

## Evolution Log
- 2026-03-01: Frame created from learning.json (2 observations)

