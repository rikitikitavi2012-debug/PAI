...
// Audio playback queue — prevents overlapping voice when multiple notifications arrive simultaneously
// (e.g. rapid Algorithm phase announcements: OBSERVE→THINK→PLAN curls in quick succession)
...
async function generateSpeech(
  ...
  if (!response.ok) {
    ...
    // Fallback: if 402 (paid voice required) and we're not already using default, retry with default
    if (response.status === 402 && voiceId !== DEFAULT_VOICE_ID) {
      console.log(`⚠️  Voice ${voiceId} requires paid plan — falling back to default: ${DEFAULT_VOICE_ID}`);
      const fallbackUrl = `https://api.elevenlabs.io/v1/text-to-speech/${DEFAULT_VOICE_ID}`;
...
// Play audio using platform-appropriate player (macOS: afplay, Linux: mpv)
async function playAudio(audioBuffer: ArrayBuffer, volume: number = FALLBACK_VOLUME): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    let proc;
    if (isLinux) {
      // mpv: volume is 0-100 scale, map from 0-1 range
      const mpvVolume = Math.round(volume * 100);
      proc = spawn('/usr/bin/mpv', ['--no-video', '--no-terminal', `--volume=${mpvVolume}`, tempFile]);
    } else {
...
async function sendNotification(
  ...
  // osascript is macOS-only — skip on Linux/WSL
  if (voiceConfig.desktopNotifications && !isLinux) {
    try {
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/notify" && req.method === "POST") {
      try {
        ...
        // Enqueue audio playback — responds immediately, plays sequentially in background
        enqueueNotification(() => sendNotification(title, message, voiceEnabled, voiceId, voiceSettings, volume).then(() => {}));

        return new Response(
          JSON.stringify({ status: "success", message: "Notification sent", queue_depth: queueDepth }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200
          }
