...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Запускаю Reconnaissance в скилле PromptInjection для map attack surface"}' \
  > /dev/null 2>&1 &
```

Запускаю **Reconnaissance** в скилле **PromptInjection** для map attack surface...
