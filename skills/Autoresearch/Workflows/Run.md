...
## Шаг 7: Диагностическая итерация (iteration 0.5)

Перед полным запуском цикла — выполнить одну диагностическую итерацию, чтобы убедиться что изменения кода **вообще двигают метрику**.

**Алгоритм:**
1. Запомнить текущую метрику (baseline из experiments.tsv)
2. Внести одно минимальное, заведомо влияющее изменение:
   - Для performance-метрик: добавить/убрать один ресурс (картинка, скрипт, зависимость)
   - Для size-метрик: добавить/удалить один файл из бандла
   - Для count-метрик: добавить/исправить один тест-кейс
   - Для score-метрик: изменить один параметр конфигурации
3. Измерить метрику
4. Оценить:
   - **Метрика сдвинулась** (любое направление, delta > 0) → ✅ Causal link подтверждён, метрика оптимизируема. Откатить изменение, продолжить к полному циклу.
   - **Метрика не сдвинулась** (delta = 0) → ⚠️ Возможная проблема:
     - Измерение кэшировано или stale?
     - Метрика не связана с изменяемым кодом?
     - Root cause вне scope autoresearch (test pollution, external dependency)?
5. Если delta = 0 — предупредить пользователя через AskUserQuestion:
   > Диагностика: изменение кода НЕ сдвинуло метрику.
   > Варианты: (1) Попробовать другое изменение, (2) Продолжить несмотря на предупреждение, (3) Остановить — метрика неоптимизируема
6. Откатить диагностическое изменение: `git checkout -- .`

**НЕ логировать диагностическую итерацию в experiments.tsv** — это валидация, не эксперимент.

---

## Шаг 8: Запуск 8-фазного итерационного цикла
...
- [ ] Диагностическая итерация подтвердила causal link (code change → metric change)
