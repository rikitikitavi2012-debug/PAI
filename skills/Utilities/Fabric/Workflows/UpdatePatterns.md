...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Обновляю паттерны Fabric из upstream-репозитория"}' \
  > /dev/null 2>&1 &
```
...
```bash
CURRENT_COUNT=$(ls -1 ${CLAUDE_SKILL_DIR}/Patterns/ 2>/dev/null | wc -l | tr -d ' ')
...
```bash
rsync -av --delete ~/.config/fabric/patterns/ ${CLAUDE_SKILL_DIR}/Patterns/
```

### Step 5: Report Results

```bash
NEW_COUNT=$(ls -1 ${CLAUDE_SKILL_DIR}/Patterns/ 2>/dev/null | wc -l | tr -d ' ')
echo ""
...
for pattern in extract_wisdom summarize create_threat_model analyze_claims; do
  if [ -d ${CLAUDE_SKILL_DIR}/Patterns/$pattern ]; then
    echo "✓ $pattern"
  else
...
# Sync patterns
rsync -av --delete patterns/ ${CLAUDE_SKILL_DIR}/Patterns/
...
```bash
# Count patterns
ls -1 ${CLAUDE_SKILL_DIR}/Patterns/ | wc -l

# List recent additions (if patterns have dates)
ls -lt ${CLAUDE_SKILL_DIR}/Patterns/ | head -10
```
