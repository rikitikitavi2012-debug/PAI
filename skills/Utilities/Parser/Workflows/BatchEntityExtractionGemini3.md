...
const entityIndex = JSON.parse(
  fs.readFileSync('${CLAUDE_SKILL_DIR}/entity-index.json', 'utf-8')
);
...
// Main processing
const entityIndexPath = '${CLAUDE_SKILL_DIR}/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('${CLAUDE_SKILL_DIR}/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `${CLAUDE_SKILL_DIR}/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ${CLAUDE_SKILL_DIR}/entity-index.json | jq '{
