...
```bash
bun ${CLAUDE_SKILL_DIR}/Tools/Pipeline.ts \
  "<audio-file-path>" \
...
```bash
# Transcription only
bun ${CLAUDE_SKILL_DIR}/Tools/Transcribe.ts <file>

# Analysis only (requires transcript)
bun ${CLAUDE_SKILL_DIR}/Tools/Analyze.ts <transcript.json>

# Edit only (requires audio + edits)
bun ${CLAUDE_SKILL_DIR}/Tools/Edit.ts <file> <edits.json>

# Polish only (requires CLEANVOICE_API_KEY)
bun ${CLAUDE_SKILL_DIR}/Tools/Polish.ts <file>
```
