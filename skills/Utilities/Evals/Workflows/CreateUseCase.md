...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Запускаю CreateUseCase в скилле Evals для create eval use case"}' \
  > /dev/null 2>&1 &
```

Запускаю **CreateUseCase** в скилле **Evals** для create eval use case...
...
```bash
mkdir -p ${CLAUDE_SKILL_DIR}/UseCases/<name>/{test-cases,golden-outputs,prompts}
```

### Step 3: Create Config File

Create `${CLAUDE_SKILL_DIR}/UseCases/<name>/config.yaml`:
...
Create `${CLAUDE_SKILL_DIR}/UseCases/<name>/prompts/v1.0.0.md`:
...
Create test cases in `${CLAUDE_SKILL_DIR}/UseCases/<name>/test-cases/`:
...
Create `${CLAUDE_SKILL_DIR}/UseCases/<name>/README.md`:
...
\`\`\`bash
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli-run.ts --use-case <name>
\`\`\`
...
```bash
# Check structure
ls -la ${CLAUDE_SKILL_DIR}/UseCases/<name>/

# Validate config
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli.ts use-case show <name>
```
...
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli-run.ts \
  --use-case <name> \
  --test-id 001-basic \
  --verbose
```
