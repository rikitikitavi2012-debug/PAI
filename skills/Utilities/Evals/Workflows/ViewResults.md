...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Запускаю ViewResults в скилле Evals для display eval results"}' \
  > /dev/null 2>&1 &
```

Запускаю **ViewResults** в скилле **Evals** для display eval results...
...
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli.ts results \
  --use-case <name> \
  --latest
```
...
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli.ts report \
  --run-id <run-id> \
  --output ${CLAUDE_SKILL_DIR}/Results/<use-case>/<run-id>/report.md
```
...
bun run ~/.claude/Templates/Tools/RenderTemplate.ts \
  -t Evals/Report.hbs \
  -d ${CLAUDE_SKILL_DIR}/Results/<use-case>/<run-id>/results.yaml \
  -o ${CLAUDE_SKILL_DIR}/Results/<use-case>/<run-id>/report.md
```
...
```bash
cd ${CLAUDE_SKILL_DIR}/EvalServer
...
```bash
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli.ts compare \
  --run-a <run-id-1> \
  --run-b <run-id-2>
```
...
```bash
bun run ${CLAUDE_SKILL_DIR}/EvalServer/cli.ts trend \
  --use-case <name> \
  --days 30
```
