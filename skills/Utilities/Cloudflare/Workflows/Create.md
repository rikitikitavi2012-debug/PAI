...
## Post-Deploy Verification via MCP

After wrangler deploy succeeds, verify the worker is live using Code Mode MCP:

1. **Confirm worker exists in account:**
   ```
   MCP execute("GET /accounts/{account_id}/workers/scripts")
   ```
   Verify the worker name appears in the response.

2. **Check worker details:**
   ```
   MCP execute("GET /accounts/{account_id}/workers/scripts/{script_name}")
   ```
   Confirm the script has the expected bindings, routes, and compatibility date.
