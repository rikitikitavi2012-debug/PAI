...
function parseOpinions(): Map<string, Opinion> {
  ...
  for (const block of opinionBlocks) {
    ...
    for (const row of tableRows) {
      const [, type, date, desc] = row.match(/\| (Supporting|Counter) \| ([^|]+) \| ([^|]+) \|/i) || [];
      if (type && desc) {
        evidence.push({
          date: date?.trim() || getISODate(),
...
/**
 * Serialize opinions map back to OPINIONS.md
 * Groups by category, each opinion as a ### block with **Category:** tag for reliable parsing
 */
function saveOpinions(opinions: Map<string, Opinion>): void {
  ...
  for (const [category, opinionList] of categories) {
    ...
    for (const op of opinionList.sort((a, b) => b.confidence - a.confidence)) {
      ...
      if (op.evidence.length > 0) {
        lines.push('');
        lines.push('| Type | Date | Description |');
        lines.push('|------|------|-------------|');
        for (const e of op.evidence) {
          lines.push(`| ${e.type.charAt(0).toUpperCase() + e.type.slice(1)} | ${e.date} | ${e.description} |`);
        }
      }
      lines.push('');
    }
