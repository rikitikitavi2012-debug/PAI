...
function parseFrontmatter(content: string): { name: string; description: string } | null {
  ...
  // Extract compatibility.min_model
  const minModelMatch = frontmatter.match(/^compatibility:\s*\n\s+min_model:\s*(.+)$/m);
  const minModel = (minModelMatch ? minModelMatch[1].trim() : 'sonnet') as 'haiku' | 'sonnet' | 'opus';

  return { name, description, minModel };
}
...
async function parseSkillFile(filePath: string): Promise<SkillEntry | null> {
  try {
    ...
    return {
      ...
      workflows,
      tier,
      minModel: frontmatter.minModel ?? 'sonnet',
    };
  } catch (error) {
