...
describe('QuestionAnswered', () => {
  const hook = 'hooks/QuestionAnswered.hook.ts';
  let tempDir: string;

  beforeEach(() => {
    tempDir = createTempDir('pai-question-answered-');
  ...
  test('runs without crash, exits 0', async () => {
    const result = await runHook(
      hook,
      {
        session_id: 'test-session',
        tool_input: {
          answer: 'It seems to be a caching issue.'
        }
      },
