...
describe('AutoSkillProposal', () => {
  ...
  test('skips if complexity threshold (< 8 tool calls) is not met', async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stderr).toContain('[AutoSkillProposal] Session too simple (3 tool calls), skipping');
  });

  test('skips if rate limit reached (already proposed this session)', async () => {
  ...
  test('detects pattern and creates skill directly', async () => {
    // 8+ tool calls
    const message = `
      ━━━ THINK
      Read(file1.txt)
    ...
    // Create mock Inference.ts that outputs a valid proposal
    const inferencePath = join(toolsDir, 'Inference.ts');
    writeFileSync(inferencePath, `#!/usr/bin/env bun
console.log(JSON.stringify({
  ...
  pattern: "Test pattern"
    ...
    // Check that skill was created directly
