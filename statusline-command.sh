...

{
    ...
    if [ "$cache_age" -gt "$USAGE_CACHE_TTL" ]; then
        ...
        if [ "$(uname)" = "Darwin" ]; then
            ...
            token=$(python3 -c "import json; d=json.load(open('$HOME/.claude/.credentials.json')); print(d.get('claudeAiOauth',{}).get('accessToken',''))" 2>/dev/null)
        fi
...
def clock_time(ts, fmt):
    ...
    try:
        _tz = _json.load(open('$SETTINGS_FILE')).get('principal',{}).get('timezone','UTC')
    except: _tz = 'UTC'
    local_dt = dt.astimezone(ZoneInfo(_tz))
    if fmt == 'weekly':
