...
export async function captureFailure(input: FailureCaptureInput): Promise<string | null> {
  ...
  // 1. Copy raw transcript and compress to save space (~60-70% reduction)
  const transcriptDest = join(failureDir, 'transcript.jsonl');
  copyFileSync(transcriptPath, transcriptDest);
  const gzResult = spawnSync('gzip', ['-9', transcriptDest], { timeout: 30000 });
  if (gzResult.status === 0) {
    console.error(`[FailureCapture] Compressed transcript: ${transcriptDest}.gz`);
  } else {
    console.error(`[FailureCapture] gzip failed, keeping uncompressed: ${gzResult.stderr?.toString()}`);
  }
