...
**v3.6.0 additions:** ISC-Metric Mapping ([B]/[Q] tagging), Experiments TSV, Dual-Track Learning, Verification Rehearsal. Inspired by Karpathy autoresearch — constraint as enabler.
...
**Rules:** One row per experiment. `commit` = `-` for discarded changes. `delta` = change from previous kept value. `status` = `baseline | keep | discard | crash | skip`. This file is the empirical memory of what worked and what didn't.
...
- ISC-METRIC MAPPING (after ISC Count Gate passes):

Tag each ISC criterion in the PRD with its verification type:
- **`[B]`** (binary) — qualitative, pass/fail. "Hero section visible", "API returns 200".
- **`[Q]`** (quantitative) — has a scalar metric. "Lighthouse > 90", "test coverage > 80%".

For each `[Q]` criterion, append inline: `| metric: <name> | cmd: <shell command> | baseline: <N> | target: <N> | direction: higher/lower |`

Example in PRD:
```
- [ ] ISC-4 [B]: Navigation links resolve without 404
- [ ] ISC-5 [Q]: Lighthouse performance score > 90 | metric: lighthouse_perf | cmd: lighthouse --output=json | jq '.categories.performance.score*100' | baseline: 62 | target: 90 | direction: higher |
```

`[B]` criteria serve as **regression gates** during any iterative optimization — if a `[Q]` optimization breaks a `[B]` criterion, the change is rejected.
`ISC-A` (anti-criteria) serve as **hard stops** — violation halts the optimization loop entirely.

If ALL criteria are `[B]`, skip this step — no mapping needed for standard execution.
...
**Track 2 — Empirical** (when experiments.tsv exists):
...
**Track 3 — Synthesis** (when Track 2 has data — the highest-value learning):
 ...
 Write cross-domain patterns to Wisdom Frames if applicable.
