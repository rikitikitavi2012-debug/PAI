...
| Tier | Default Cap | Configurable Range | Approx Time |
|------|-------------|-------------------|-------------|
| **Standard** | 20 | 10-30 | 15-30 min |
| **Extended** | 50 | 30-80 | 45-90 min |
| **Advanced** | 75 | 50-120 | 1-3 hours |
| **Deep** | 100 | 80-200 | 2-8 hours |
| **Comprehensive** | 200 | 150-500 | 4-17 hours |

Override default cap in PRD frontmatter: `iteration_cap: 80`. Time budget = remaining Effort Level time minus reserve for VERIFY + LEARN.
...
**Routing rules:**
1. If ISC-Metric Mapping was skipped (Standard tier) → Standard EXECUTE (default)
2. If all criteria are `[B]` → Standard EXECUTE
3. If any `[Q]` criteria exist → check search space: single obvious fix = Standard, multiple possible approaches = Autoresearch
4. Mixed `[B]` + `[Q]` → Hybrid: Standard EXECUTE for `[B]`, then Autoresearch sub-loop for `[Q]`
...
- **Standard EXECUTE:** Work through `[B]` criteria directly. No iteration loop.
...
- **Hybrid EXECUTE:** First complete all `[B]` criteria via Standard. Then read `Algorithm-Autoresearch.md` and run the sub-loop for each `[Q]` criterion.
...
**Track 3 — Synthesis** (only when Track 2 has 5+ experiments — skip otherwise):
