...
**Multiple `[Q]` criteria:** When a PRD has 2+ `[Q]` criteria, optimize them **sequentially** (one at a time), not in parallel. Each `[Q]` gets its own section in experiments.tsv (separated by a header comment `# [Q] ISC-N: description`). After optimizing `[Q]-1`, its achieved value becomes a **regression gate** for `[Q]-2` (must not regress below best achieved minus 5% tolerance). If optimizing one `[Q]` fundamentally conflicts with another (metric improves on one but regresses the other beyond tolerance for 5+ consecutive attempts), STOP and present the tradeoff to the user via AskUserQuestion.
