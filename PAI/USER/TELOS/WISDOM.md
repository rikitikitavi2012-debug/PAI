...
### W10: Глубина экспертизы = конкурентный ров — кто объясняет "почему", тот эксперт
**Источник:** Построение TF knowledge base (март 2026). Конкуренты описывают "ЧТО" (фахверк, шип-паз, клеёный брус). Мы объясняем "ПОЧЕМУ" (почему ласточкин хвост на стропилах, почему шип-паз на стойках, почему TFEC а не только СП 64). Разница между продавцом и экспертом — глубина объяснения.
**Контекст:** W7 (экспертизу нельзя купить), S6 (TF фокус), G1 (бизнес TF). Knowledge base из 7 файлов + связь с Притупом + TFContent skill = системная экспертность, которую конкурент не скопирует за неделю.
